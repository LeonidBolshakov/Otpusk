...
а начисляемые на них надбавки "вторичными"

Скрипт читает файл `UCHRABVR.txt`, группирует строки по сотруднику (`clsch`),
ищет «основные» и «вторичные» виды оплат по правилу `VARIABLE_VIDOPS`,
суммирует значения и формирует список `UPDATE`‑операторов для Галактики.
...
* Конфигурация читается из `otpusk.cfg`; при отсутствии файла конфигурации
